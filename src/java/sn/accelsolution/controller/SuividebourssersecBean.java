...
import sn.accelsolution.util.UtilDs;
...
@ManagedBean
@ViewScoped
public class SuividebourssersecBean implements Serializable {
    ...
    private List<UtilDs> allUtilDs;
    private UtilDs utilDs;
    ...
    public void init() {
        ...
        pourcentageExe = 0;
        //this.allUtilDs = new ArrayList<>();
        List<Mouvementdebourse> listMouvementdebourse = ejbMouvementdebourse.findAll();
        List<Deboursser> listDebourssers = ejbDeboursser.findAll();
        List<Deboursser> listTempDebourssers = new ArrayList<>();
        listTempDebourssers.clear();

//        for (Deboursser deb : listDebourssers) {
//            int vconso = 0;
//            //this.utilDs = new UtilDs();
//
//            System.out.println("Deboursser ID: " + deb.getIdDeboursser());
//
//            if (!listTempDebourssers.contains(deb)) {
//                for (Mouvementdebourse mouv : listMouvementdebourse) {
//                    if (deb.getIdDeboursser() == mouv.getIdDeboursse().getIdDeboursser()) {
//                        vconso = vconso + mouv.getQuantite();
//                    }
//                }
//
//                for (Mouvementdebourse mouv : listMouvementdebourse) {
//                    System.out.println("Mouvement ID: " + mouv.getIdDeboursse().getIdDeboursser());
//                    if (deb.getIdDeboursser() == mouv.getIdDeboursse().getIdDeboursser()) {
//                        this.utilDs = new UtilDs();
//                        this.utilDs.setIdMouvementDeboursserSec(mouv.getIdSortiedeb());
//                        this.utilDs.setChantier(mouv.getIdChantier().getSiteChantier());
//                        this.utilDs.setDesignation(mouv.getIdDeboursse().getIdMateriel().getLibelle());
//                        this.utilDs.setNivodeboursser(mouv.getIdDeboursse().getIdNivodeboursser().getLibellenivodeboursser());
//                        this.utilDs.setQt(Integer.parseInt(deb.getQuantite()));
//                        this.utilDs.setUnite(deb.getUnite());
//                        this.utilDs.setDateMouvDeboursser(mouv.getDatesortiedeb());
//
//                        System.out.println("Avant Utils: " + this.utilDs);
//                    }
//                }
//
//                int vqt = Integer.parseInt(deb.getQuantite());
//                int p1 = vconso * vqt;
//                int p2 = p1 / 100;
//                String vvconso = String.valueOf(p2);
//                this.utilDs.setConso(String.valueOf(vconso));
//                this.utilDs.setTaux(vvconso);
//                System.out.println("Apres Utils: " + this.utilDs);
//                /*System.out.println("Conso: " + vconso);
//                System.out.println("Taux: " + vvconso);
//                System.out.println("Util DS: " + this.utilDs.getDateMouvDeboursser());*/
//                this.allUtilDs.add(this.utilDs);
//                listTempDebourssers.add(deb);
//
//            }
        //       }
    }

    public SuividebourssersecBean() {
    ...
    public UtilDs getUtilDs() {
        return utilDs;
    }

    public void setUtilDs(UtilDs utilDs) {
        this.utilDs = utilDs;
    }

    public List<UtilDs> getAllUtilDs() {
        return allUtilDs;
    }

    public void setAllUtilDs(List<UtilDs> allUtilDs) {
        this.allUtilDs = allUtilDs;
    }
    ...
    public void detail2() {
        try {
            List<Mouvementdebourse> listMouvementdebourse = ejbMouvementdebourse.listOfMouvementdebourseByChantier(this.chantier);
            List<Deboursser> listDebourssers = ejbDeboursser.findAll();
            List<Deboursser> listTempDebourssers = new ArrayList<>();
            listTempDebourssers.clear();
            this.allUtilDs = new ArrayList<>();
                ...
                for (Mouvementdebourse mouv : listMouvementdebourse) {
                    if (deb.getIdDeboursser() == mouv.getIdDeboursse().getIdDeboursser()) {
                        if (passage == 0) {
                            ...
                            for (Mouvementdebourse mouvInter : listMouvementdebourse) {
                                if (mouv.getIdDeboursse().getIdDeboursser() == mouvInter.getIdDeboursse().getIdDeboursser()) {
                                    vconso = vconso + mouv.getQuantite();
                                }
                            }
                            passage++;
                            ...
                            for (Deboursser tempdeb : listTempDebourssers) {
                                if (tempdeb.getIdDeboursser() == mouv.getIdDeboursse().getIdDeboursser()) {
                                    trouve = true;
                                    break;
                                } else {
                                    trouve = false;
                        ...
                        if (trouve == false) {
                            /*Ajout dans temp*/
                            listMouvementdebourse.add(mouv);

                            /*Ajout dans la liste util*/
                            this.utilDs = new UtilDs();
                            this.utilDs.setIdMouvementDeboursserSec(mouv.getIdSortiedeb());
                            this.utilDs.setChantier(mouv.getIdChantier().getSiteChantier());
                            this.utilDs.setDesignation(mouv.getIdDeboursse().getIdMateriel().getLibelle());
                            ...
                            System.out.println("Deja traiter !!!");
                        }
